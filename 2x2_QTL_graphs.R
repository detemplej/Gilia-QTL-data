gmap <- read.cross(format = "csv", file = "C:/Users/detemplj/OneDrive - Iowa State University/Documents/Research (Whipple Lab)/My_paper/gmap5335_final.csv", map.function = "c-f", genotypes = c("AA", "AB", "BB"), alleles = c("A", "B"))
#################################################################################################################
...
for (i in 1:5){
  ...
  par(mfrow = c(2, 2))
  plot(out.ehk1, main = colnames(gmap_qtl$pheno)[specific_index], cex.main = 2, cex.lab = 1.5)
  abline(h = 4.16, col = "blue", lty = 2)
  plot(out.ehk2, main = colnames(gmap_qtl$pheno)[specific_index], cex.main = 2, cex.lab = 1.5)
  abline(h = 4.16, col = "blue", lty = 2)
  plot(out.ehk3, main = colnames(gmap_qtl$pheno)[specific_index], cex.main = 2, cex.lab = 1.5)
  abline(h = 4.16, col = "blue", lty = 2)
  plot(out.ehk4, main = colnames(gmap_qtl$pheno)[specific_index], cex.main = 2, cex.lab = 1.5)
  abline(h = 4.16, col = "blue", lty = 2)
  dev.off()
}
