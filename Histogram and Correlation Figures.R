...
## Quick notes: this file produces two figures: 1. Histogram figure and
## 2. Correlation plot. As these both depend on the same phenotypic data, it was
## easier to combine them into a single script
...
## Outputting to a high-quality image file

png(filename = "Histograms.png", res=400, width = 5000, height = 4000)
plot +                                                 # Add different lines to facet plot
  geom_vline(data = yor_sum,
             aes(xintercept = vline, color = "blue"), lwd = 2,
             alpha = 0.5, show.legend = F) +
...
png(filename = "Correlations.png", res=500, width = 5000, height = 4000)
corrplot(f2_corr_na, type = "upper", method = "circle", order = "original", diag = F, addgrid.col = 'white', tl.col = "black", tl.cex = 0.8, tl.srt = 60)
dev.off()
